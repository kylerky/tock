...
static __attribute__((always_inline)) struct metadata* get_shadow_addr_inner(void *addr) {
    // return (void*)(( (unsigned long)addr - 0x0 ) * 4 + 0x88000000);
    // return (void*)(( (unsigned long)addr - 0x0 ) * 4 + 0x2000d000);
...
void __instrument_memmove(void **dst, void **src, size_t len) {
    ...
    const size_t mask = (1 << POINTER_ALIGNMENT_P2) - 1;
    if (((size_t)src & mask) != 0 || ((size_t)dst & mask) != 0 || (len & mask) != 0)
        return;
    len >>= POINTER_ALIGNMENT_P2;

    while (len > 0) {
        size_t src_stage2_entries = STAGE2_ENTRIES - INDEX_STAGE2(src);
        size_t dst_stage2_entries = STAGE2_ENTRIES - INDEX_STAGE2(dst);
        size_t chunk_size = min_usize(len, min_usize(src_stage2_entries, dst_stage2_entries));

        void *src_ptr = get_shadow_addr_inner(src);
        void *dst_ptr = get_shadow_addr_inner(dst);
        __builtin_memmove(dst_ptr, src_ptr, chunk_size << METADATA_SIZE_P2);
